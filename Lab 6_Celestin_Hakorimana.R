...
iris %>%
  ...
  arrange(desc(mean_Petal_Len)) -> iris_final
###################################
####    Linear models intro    ####
###################################
...
#The intercept represents the estimated fuel efficiency for a hypothetical vehicle with an engine displacement of 0 cubic inches.
#t is approximately 29.6 mpg. Since an engine size of 0 is physically impossible, this value is purely theoretical and serves as the baseline for the regression line.
### 4.2
# Interpret the Estimate value in the disp row.
#the fuel efficiency of the car decreases by an average of 0.041 miles per gallon.
### 4.3
# Is the relationship between these two variables positive or negative? Why do you think that might be?
#The relationship is negative (inverse). As engine size (displacement) increases, fuel efficiency (mpg) decreases.
...
mtcars$disp_centred <- mtcars$disp - mean(mtcars$disp)
mean(mtcars$disp_centred)
var(mtcars$disp_centred)

# mean returns effectively -1.199041e-14
# var returns = 15360
### 4.6
...
# intercept has changed to 20.09
### 4.7
...
coef(model_cc)
mean_cc <- mean(mtcars$disp_cc)
# prediction for 6500cc:
deviation <- 6500 - mean_cc
pred_kml <- coef(model_cc)[1] + coef(model_cc)[2] * deviation
print(pred_kml)
# the fuel effiency is 5.55km/l
### 4.8
