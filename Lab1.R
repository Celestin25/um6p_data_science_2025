...
rolling_dice <- function(x) {
  die1 <- sample(1:6,x, replace = TRUE)
  die2 <- sample(1:6,x, replace = TRUE)
  return(die1 + die2)
}
### 4.2
...
#What I noticed as the number of rolls increases, the distribution becomes smoother and more clearly
...
#The probability density is 1 for all values between 0 and 1 because the area under the pdf must be equal to 1
### 4.5
...
# the probability density is 1 for all values between 5 and 6
### 4.6
...
#The probability densityis 2 for all values between 0 and 0.5
### 4.7
...
#The probability density is 0.5 for all values between 0 and 2
### 4.8
...
#The dunif function returns the probability density value for a uniform distribution.