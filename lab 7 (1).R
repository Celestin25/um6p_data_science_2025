...
ggplot(mtcars, aes(x = wt , y = mpg ,  shape = factor(cyl), size =disp )) + geom_point()
#################################
####    Geoms and Layers     ####
#################################
...
ggplot(mtcars,aes(x = wt) )+
  geom_histogram(fill= "#FF0000", col="blue")
...
ggplot(mtcars,aes(x = wt, y= cyl) )+
  geom_boxplot(fill= "#FF0000", col="blue")
...
ggplot(mtcars,aes(x = wt, y = mpg, fill=factor(am)),  col=factor(am))+
  geom_point(alpha = 0.6) +
...
ggplot(mtcars, aes(wt, mpg)) +
  ...
  scale_y_continuous(
    breaks = seq(10, 35, 5),
    labels = paste0(seq(10, 35, 5), " mpg (", seq(10, 35, 5)*0.425144, " km/l)"  )       # Format y-axis labels
  )
...
lines(x,model$coefficients[1,]+x*model$coefficients['x',])
